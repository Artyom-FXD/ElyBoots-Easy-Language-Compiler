...
public:
    explicit function(::ElyHeapFunction* f) : raw_(f) {
        if (!raw_) {
            throw ErrorException(ErrorType::ValueError, "ValueError: Function initialization pointer cannot be null");
        }
    }

    explicit function(ely_value val) {
        if (!ely_is_ptr(val)) {
            throw ErrorException(ErrorType::TypeError, "TypeError: Expected GC pointer to load Function");
        }
        auto* obj = static_cast<ElyHeapObject*>(ely_as_ptr(val));
        if (!obj || static_cast<uint8_t>(obj->type) != ELY_HEAP_FUNCTION) {
            throw ErrorException(ErrorType::TypeError, "TypeError: Passed ely_value does not refer to a Function object!");
        }
